// Package scrabblescorer provides a Go client for the Scrabble Word Scorer API.
//
// For more information, visit: https://apiverve.com/marketplace/scrabblescorer?utm_source=go&utm_medium=readme
...
// Request contains the parameters for the Scrabble Word Scorer API.
//
// Parameters:
//   - word (required): string - The word to score
//   - language: string - Scrabble language variant: english, french, spanish, german, or italian (default: english)
type Request struct {
...
// ResponseData contains the data returned by the Scrabble Word Scorer API.
type ResponseData struct {
	Word string `json:"word"`
	Language string `json:"language"`
	TotalScore int `json:"total_score"`
	LetterCount int `json:"letter_count"`
	LetterScores []LetterScoresItem `json:"letter_scores"`
	HighestScoringLetter HighestScoringLetterData `json:"highest_scoring_letter"`
	AverageLetterScore float64 `json:"average_letter_score"`
	Note string `json:"note"`
}

// LetterScoresItem represents an item in the LetterScores array.
type LetterScoresItem struct {
	Letter string `json:"letter"`
	Score int `json:"score"`
}

// HighestScoringLetterData represents the highest_scoring_letter object.
type HighestScoringLetterData struct {
	Letter string `json:"letter"`
	Score int `json:"score"`
}
